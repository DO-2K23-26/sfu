...
use bytes::Bytes;
use serde::Serialize;
use sfu::RTCSessionDescription;
use tracing::{error, info};

use crate::{SignalingMessage, SignalingProtocolMessage};
...
pub async fn index() -> impl Responder {
    let chat_document = include_str!("chat.html");
    info!("chat_document: {}", chat_document);
    HttpResponse::Ok().body(chat_document)
}
